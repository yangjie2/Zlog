...
void AutoBuffer::__FitSize(size_t _len) {
    if (_len > capacity_) {
        ...
        if (NULL == p) {
            zl_printf("AutoBuffer::__FitSize, p == NULL; _len=%" PRIu64 ", m_nMallocUnitSize=%" PRIu64 ", nMallocSize=%" PRIu64", m_nCapacity=%" PRIu64,(uint64_t)_len, (uint64_t)malloc_unitsize_, (uint64_t)mallocsize, (uint64_t)capacity_);
            free(parray_);
            parray_ = NULL;
            capacity_ = 0;
            return;
