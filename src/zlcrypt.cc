...
ZLogCrypt::ZLogCrypt(const char* _pubkey): seq_(0), is_crypt_(false) {
...
uint32_t ZLogCrypt::GetHeaderLen() {
    return sizeof(char) * 3 + sizeof(uint16_t) + sizeof(uint32_t) + sizeof(char) * 64;
}

uint32_t ZLogCrypt::GetTailerLen() {
    return sizeof(kMagicEnd);
}

bool ZLogCrypt::GetLogHour(const char* const _data, size_t _len, int& _begin_hour, int& _end_hour) {
...
void ZLogCrypt::UpdateLogHour(char* _data) {
...
uint32_t ZLogCrypt::GetLogLen(const char*  const _data, size_t _len) {
...
void ZLogCrypt::UpdateLogLen(char* _data, uint32_t _add_len) {
...
void ZLogCrypt::SetHeaderInfo(char* _data, bool _is_async) {
    if (_is_async) {
        if (is_crypt_) {
...
void ZLogCrypt::SetTailerInfo(char* _data) {
    memcpy(_data, &kMagicEnd, sizeof(kMagicEnd));
}

bool ZLogCrypt::GetPeriodLogs(const char* const _log_path, int _begin_hour, int _end_hour, unsigned long& _begin_pos, unsigned long& _end_pos, std::string& _err_msg) {
...
void ZLogCrypt::CryptSyncLog(const char* const _log_data, size_t _input_len, AutoBuffer& _out_buff) {
...
void ZLogCrypt::CryptAsyncLog(const char* const _log_data, size_t _input_len, AutoBuffer& _out_buff, size_t& _remain_nocrypt_len) {
...
bool ZLogCrypt::Fix(char* _data, size_t _data_len, bool& _is_async, uint32_t& _raw_log_len) {
