...
ZLogBuffer::ZLogBuffer(void* _pbuffer, size_t _len, bool _isCompress, const char* _pubkey)
: is_compress_(_isCompress), log_crypt_(new ZLogCrypt(_pubkey)), remain_nocrypt_len_(0) {
...
PtrBuffer& ZLogBuffer::GetData() {
    return buff_;
}


void ZLogBuffer::Flush(AutoBuffer& _buff) {
...
bool ZLogBuffer::Write(const void* _data, size_t _inputlen, AutoBuffer& _out_buff) {
...
bool ZLogBuffer::Write(const void* _data, size_t _length) {
